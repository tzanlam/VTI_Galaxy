-- Tạo schema
CREATE SCHEMA IF NOT EXISTS vti_movie_booking_ticket;

-- Chuyển đến schema
SET search_path TO vti_movie_booking_ticket;

-- Bảng cinema
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.cinema (
...
-- Bảng account
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.account (
    ...
    FOREIGN KEY (cinema_id) REFERENCES vti_movie_booking_ticket.cinema (cinema_id) ON DELETE SET NULL
);

CREATE INDEX idx_email ON vti_movie_booking_ticket.account (email);

-- Bảng room
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.room (
    room_id SERIAL PRIMARY KEY,
    name VARCHAR(45) NOT NULL,
    type_screen VARCHAR(45),
    cinema_id INT,
    FOREIGN KEY (cinema_id) REFERENCES vti_movie_booking_ticket.cinema (cinema_id) ON DELETE SET NULL
);

-- Bảng seat
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.seat (
    seat_id SERIAL PRIMARY KEY,
    seat_name VARCHAR(10) NOT NULL,
    seat_price DECIMAL(10,2),
    seat_type TEXT CHECK (seat_type IN ('STANDARD', 'VIP', 'COUPLE')),
    seat_num INT
);

-- Bảng seat_room
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.seat_room (
    seat_room_id SERIAL PRIMARY KEY,
    seat_id INT,
    room_id INT,
    status TEXT CHECK (status IN ('AVAILABLE', 'BOOKED', 'BROKEN')) DEFAULT 'AVAILABLE',
    price DECIMAL(10,2),
    FOREIGN KEY (seat_id) REFERENCES vti_movie_booking_ticket.seat (seat_id) ON DELETE SET NULL,
    FOREIGN KEY (room_id) REFERENCES vti_movie_booking_ticket.room (room_id) ON DELETE SET NULL
);

-- Bảng movie
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.movie (
...
-- Bảng showtime
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.showtime (
    ...
    FOREIGN KEY (room_id) REFERENCES vti_movie_booking_ticket.room (room_id) ON DELETE SET NULL,
    FOREIGN KEY (movie_id) REFERENCES vti_movie_booking_ticket.movie (movie_id) ON DELETE SET NULL,
    CONSTRAINT chk_time CHECK (end_time > start_time)
);

-- Bảng payment
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.payment (
    payment_id SERIAL PRIMARY KEY,
    method TEXT CHECK (method IN ('CASH', 'CARD', 'ONLINE')) NOT NULL,
...
-- Bảng ticket
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.ticket (
    ...
    FOREIGN KEY (account_id) REFERENCES vti_movie_booking_ticket.account (account_id) ON DELETE SET NULL,
    FOREIGN KEY (showtime_id) REFERENCES vti_movie_booking_ticket.showtime (showtime_id) ON DELETE SET NULL,
    FOREIGN KEY (seat_room_id) REFERENCES vti_movie_booking_ticket.seat_room (seat_room_id) ON DELETE SET NULL,
    FOREIGN KEY (payment_id) REFERENCES vti_movie_booking_ticket.payment (payment_id) ON DELETE SET NULL
);

-- Bảng employee
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.employee (
...
-- Bảng statistical
CREATE TABLE IF NOT EXISTS vti_movie_booking_ticket.statistical (
    statistical_id SERIAL PRIMARY KEY,
    cinema_id INT,
...
INSERT INTO vti_movie_booking_ticket.cinema (cinema_name, address, status, phone_number) VALUES
...
INSERT INTO vti_movie_booking_ticket.account (fullname, role, email, cinema_id, status, created_at) VALUES
...
INSERT INTO vti_movie_booking_ticket.room (name, type_screen, cinema_id) VALUES
('Room 1', '2D', 1),
('Room 2', '3D', 1),
('Room 1', '2D', 2),
('Room 2', 'IMAX', 2),
...
INSERT INTO vti_movie_booking_ticket.seat (seat_name, seat_price, seat_type, seat_num) VALUES
...
INSERT INTO vti_movie_booking_ticket.movie (movie_name, genre, duration, release_date, actor, director, description) VALUES
...
-- Xóa và làm trống bảng showtime
DELETE FROM vti_movie_booking_ticket.showtime;
TRUNCATE TABLE vti_movie_booking_ticket.showtime;

-- Chèn dữ liệu vào bảng showtime
INSERT INTO vti_movie_booking_ticket.showtime (room_id, movie_id, start_time, end_time, status) VALUES
-- Ngày 2025-03-22
...
-- Chèn dữ liệu vào bảng seat_room
INSERT INTO vti_movie_booking_ticket.seat_room (seat_id, room_id, status, price) VALUES
...
INSERT INTO vti_movie_booking_ticket.payment (method, amount, status, created_at) VALUES
...
INSERT INTO vti_movie_booking_ticket.ticket (account_id, showtime_id, seat_room_id, total_price, payment_id, status, booking_date) VALUES
...
-- Truy vấn
-- Các truy vấn trong PostgreSQL gần giống MySQL, nhưng có một số thay đổi nhỏ:

-- CURDATE() trong MySQL được thay bằng CURRENT_DATE trong PostgreSQL.
-- Cú pháp JOIN và ORDER BY không thay đổi.
-- Dưới đây là các truy vấn đã chuyển đổi:

-- 1.Danh sách giờ chiếu của một phim:
SELECT 
    s.showtime_id,
    s.start_time,
    s.end_time,
    ...
    r.name AS room_name
FROM 
    vti_movie_booking_ticket.showtime s
JOIN 
    vti_movie_booking_ticket.movie m ON s.movie_id = m.movie_id
JOIN 
    vti_movie_booking_ticket.room r ON s.room_id = r.room_id
ORDER BY 
    s.start_time ASC;

-- 2.Kiểm tra phòng trống:
SELECT COUNT(*) 
FROM vti_movie_booking_ticket.showtime
WHERE room_id = 1
AND (
...
-- 5.Gợi ý giờ trống:
SELECT r.room_id
FROM vti_movie_booking_ticket.room r
