...
export const fetchSeatBooked = createAsyncThunk("seatBooked/fetchByRoomAndTime", async({roomId, time}, {rejectWithValue})=>{
    try {
        return (await seatBookedService.fetchSeatBooked(roomId, time)).data
    } catch (error) {
        return rejectWithValue(error.data?.message || "lỗi khi fetch trạng thái ghế")
    }
})
